...
A file containing ``to be or not to be that is the question'' will thus return:
\begin{pythonOutput}
      2 to
      2 be
      1 the
...
%By just using a BoW representation of the request_text, we can predict the accuracy by doing a sum(logistic.predict(X_test) == y_test)/len(y_test), we can also use the score method, we get the following score: SKAL INDSÆTTES
...
We implemented the DBSCAN algorithm using a combination of Numpy and Cython. We used memoization for the distances as soon as we computed them to avoid unnecessary calls to the jaccard distance method.
The memoization of distances will not work for the 100000x100000 dataset due to memory allocation, thus the execution for that one is without memoization.
