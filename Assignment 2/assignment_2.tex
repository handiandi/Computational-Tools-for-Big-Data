...
\begin{enumerate}
  \item "Selects" the customer, Order and products nodes \ldots
...
~\\
\textbf{Explanation of code and discussion}\\
We can see from the output graph, that we get a result which is quite similar to the result from week 5. In this result it is way better to see how it is connected. ~\\
Besides giving the edges "names" we have also given the nodes names, from a menu in Graphstory.com
...
\begin{enumerate}
  ...
  \item Last we return the list of customerID and the length of the list
\end{enumerate}
~\\
\textbf{Code}
...
~\\
We chose to show the result as a table and not a graph (like the latter exercises), as we didn't see the benefit of a graph in this exercise. There is no connection between the customers which is importent for the exercise. 
...
\begin{enumerate}
   \item "Selects" the Customer, Order and Products nodes
  ...
  \item Last we return the list of ProductNames and the length of that list\\
\end{enumerate}
~\\
\textbf{Code}
